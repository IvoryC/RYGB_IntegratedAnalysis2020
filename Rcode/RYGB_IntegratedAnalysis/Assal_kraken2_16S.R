...
input<-"./input/RYGB_Assal2020/Kraken2/"
output<-"./output/MixedLinearModels/Kraken2/"
taxa<-c("Phylum","Class","Order","Family","Genus","Species")

for (t in taxa){
  
  myT<-read.table(paste0(input,"/RYGB_Assal_2020Apr23_taxaCount_norm_Log10_",t,".tsv"),
                  sep="\t",header = TRUE,row.names = 1,check.names = FALSE)
  
  meta<-read.table(paste0(input,"SraRunTable_Assal.txt"),sep=",",header = TRUE)
